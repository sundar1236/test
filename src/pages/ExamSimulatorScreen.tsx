...
export const ExamSimulatorScreen: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#F8FAFC] dark:bg-[#0B0F17] text-slate-900 dark:text-slate-100 flex flex-col font-sans overflow-hidden select-none">
      ...
      {/* Top Header - High Contrast Dark Banking Header Bar */}
      <header className="h-14 sm:h-16 border-b border-[#0B3A64] dark:border-slate-800 bg-[#0F4C81] dark:bg-[#151D2A] px-3 sm:px-6 flex items-center justify-between shrink-0 shadow-md">
        <div className="flex items-center gap-2.5 min-w-0">
          <span className="px-2.5 py-1 rounded-lg bg-white/20 dark:bg-[#38BDF8]/25 text-white dark:text-[#38BDF8] font-black text-xs sm:text-sm tracking-wide shrink-0 border border-white/30 dark:border-[#38BDF8]/40 shadow-xs">
            {testMeta.exam}
          </span>
      ...
      {/* Section Tabs */}
      <div className="border-b border-slate-200 dark:border-slate-800 bg-slate-100 dark:bg-[#151D2A]/90 px-3 sm:px-6 flex items-center gap-2 overflow-x-auto shrink-0 py-1">
        <span className="text-[10px] sm:text-xs font-black text-slate-700 dark:text-slate-300 uppercase tracking-wider mr-1 shrink-0">Sections:</span>
          ...
          <button
            ...
            className={`px-4 sm:px-5 py-2 sm:py-2.5 text-xs sm:text-sm font-black rounded-xl transition-all whitespace-nowrap ${
              activeSectionName === secName
                ? 'bg-[#0F4C81] dark:bg-[#38BDF8] text-white dark:text-slate-950 shadow-sm'
                : 'bg-white/60 dark:bg-slate-800/60 text-slate-800 dark:text-slate-200 hover:bg-white dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700'
            }`}
          >
            {secName}
          </button>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
        ...
        <div className="flex-1 flex flex-col p-4 sm:p-6 md:p-8 overflow-y-auto bg-white dark:bg-[#0B0F17]">

          <div className="max-w-5xl w-full mx-auto flex-1 flex flex-col">
            {/* Question Header Status Bar */}
            <div className="flex items-center justify-between mb-4 sm:mb-6 pb-3 border-b border-slate-200 dark:border-slate-800">
              <div className="flex items-center gap-2 min-w-0">
                <span className="text-base sm:text-xl font-black text-[#0F4C81] dark:text-[#38BDF8] shrink-0">
                  Q{currentQuestionIndex + 1}
                </span>
                <span className="text-xs sm:text-sm text-slate-800 dark:text-slate-200 font-extrabold truncate">
                  of {sectionQuestions.length} ({activeSectionName})
                </span>
              </div>

              <div className="flex items-center gap-2 shrink-0">
                <span className="text-[10px] sm:text-xs font-black px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-700">
                  +1.0 / -0.25
                </span>
                <button
            ...
            {/* Question Body */}
            <div className="space-y-5 sm:space-y-6 flex-1 pb-24 md:pb-6">
              <div className="text-base sm:text-lg md:text-xl font-semibold text-slate-900 dark:text-slate-100 leading-relaxed pr-1">
                ...
                {currentQuestion.options.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full p-4 sm:p-5 rounded-2xl border-2 text-left text-sm sm:text-base font-medium transition-all flex items-start sm:items-center gap-3.5 sm:gap-4 shadow-2xs ${
                        isSelected
                          ? 'border-[#0F4C81] dark:border-[#38BDF8] bg-[#0F4C81]/10 dark:bg-[#38BDF8]/20 text-[#0F4C81] dark:text-[#38BDF8] font-bold ring-2 ring-[#0F4C81]/30 dark:ring-[#38BDF8]/40'
                          : 'border-slate-300 dark:border-slate-700 bg-white dark:bg-[#151D2A] hover:border-[#0F4C81] dark:hover:border-[#38BDF8] text-slate-900 dark:text-slate-100'
                      }`}
                    >
                      <span className={`w-8 h-8 sm:w-9 sm:h-9 rounded-lg flex items-center justify-center font-black text-xs sm:text-sm shrink-0 mt-0.5 sm:mt-0 transition-colors ${
                        isSelected
                          ? 'bg-[#0F4C81] dark:bg-[#38BDF8] text-white dark:text-slate-950 shadow-xs'
                          : 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-300 dark:border-slate-600'
                      }`}>
                        {isSelected ? <Check className="w-4 h-4 sm:w-5 sm:h-5 stroke-[3]" /> : opt.option_key}
                      </span>
                      <span className="flex-1 leading-relaxed text-slate-900 dark:text-slate-100 text-sm sm:text-base font-medium">{opt.text}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <div className="hidden md:flex pt-4 mt-auto border-t border-slate-200 dark:border-slate-800 items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-4 py-2.5 rounded-xl border border-purple-400 dark:border-purple-600 bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-300 font-bold text-xs hover:bg-purple-100 transition-colors"
                >
                  Mark For Review
                </button>
                <button
                  onClick={handleClearResponse}
                  className="px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 font-semibold text-xs transition-colors"
                >
                  Clear Response
                </button>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-4 py-2.5 rounded-xl border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-200 font-bold text-xs disabled:opacity-40 flex items-center gap-1"
                >

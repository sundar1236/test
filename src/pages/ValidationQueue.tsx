...
import { Check, X, Edit3, Sparkles, AlertCircle, FileCheck2 } from 'lucide-react';
...
export const ValidationQueue: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-extrabold text-[var(--text-main)] tracking-tight">AI Question Validation Queue</h1>
        <p className="text-sm text-[var(--text-muted)] mt-1">
          Review automated AI parsing and verify answer accuracy before pushing questions live to students.
        </p>
      </div>
      ...
      {pendingQuestions.length === 0 ? (
        <div className="p-12 text-center rounded-2xl bg-[var(--bg-card)] border border-[var(--border-color)] space-y-3">
          ...
          <h3 className="font-bold text-base text-[var(--text-main)]">Queue is completely clear!</h3>
          <p className="text-xs text-[var(--text-muted)]">
            All imported banking questions have been audited and published to active mock test series.
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex items-center justify-between gap-2 flex-wrap">
                ...
                <div className="flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-lg bg-purple-500/10 text-purple-600 dark:text-purple-400">
                  <Sparkles className="w-3.5 h-3.5" /> AI Confidence: {q.aiConfidence || 90}%
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-medium">
                {q.options.map((opt) => (
                  <div key={opt.id} className="p-2.5 rounded-lg border border-[var(--border-color)] bg-[var(--bg-main)]">
              ...
              <div className="pt-2 flex items-center justify-end gap-3">
                ...
                >
                  <X className="w-4 h-4" /> Reject Question
                </button>
                <button
